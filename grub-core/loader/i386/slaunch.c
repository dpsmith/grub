		...
		int argc, char *argv[])
{
  ...
  if (grub_errno == GRUB_ERR_NONE)
    {
      grub_loader_set (grub_slaunch_boot, grub_slaunch_unload,
...
static grub_err_t
grub_cmd_acm (grub_command_t cmd __attribute__ ((unused)),
		 int argc, char *argv[])
{
...
static grub_err_t
grub_cmd_slb (grub_command_t cmd __attribute__ ((unused)),
		 int argc, char *argv[])
{
  grub_size_t size = 0, aligned_size = 0;
  grub_addr_t addr_min, addr_max;
...
static grub_command_t cmd_slaunch, cmd_acm, cmd_slb;

GRUB_MOD_INIT(linux)
{
  cmd_slaunch = grub_register_command ("slauch", grub_cmd_slaunch,
				     0, N_("Load Secure Launcher."));
  cmd_acm = grub_register_command ("acm", grub_cmd_acm,
				      0, N_("Load Intel ACM."));
  cmd_slb = grub_register_command ("slb", grub_cmd_slb,
				      0, N_("Load AMD SLB."));
  my_mod = mod;
}
...
GRUB_MOD_FINI(linux)
{
  ...
  grub_unregister_command (cmd_acm);
  grub_unregister_command (cmd_slb);
}
